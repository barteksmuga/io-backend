...
metadata blocks}. Metadata values specified inside the document, or by
using \texttt{-M}, overwrite values specified with this option.
...
selected), Markdown (which uses entities), roff ms (which use
hexadecimal escapes), and to a limited degree LaTeX (which uses standard
commands for accented characters when possible). roff man output uses
ASCII by default.
...
\texttt{gfm} output, ATX headers are always used.)
...
document class,
e.g.~\href{https://ctan.org/pkg/article}{\texttt{article}},
...
multiple options
\item[\texttt{mainfont}, \texttt{romanfont}, \texttt{sansfont},
\texttt{monofont}, \texttt{mathfont}, \texttt{CJKmainfont}]
...
available.
\item[\texttt{mainfontoptions}, \texttt{romanfontoptions},
\texttt{sansfontoptions}, \texttt{monofontoptions},
\texttt{mathfontoptions}, \texttt{CJKoptions}]
...
\href{https://wiki.contextgarden.net/Layout}{ConTeXt Layout}); may be
...
Headers and Footers}); may be repeated up to four times for different
placement
...
\item
  ...
  using YAML metadata or the \texttt{-\/-metadata} flag, use
  \texttt{true}, \texttt{True}, or \texttt{TRUE}; with the
  \texttt{-\/-variable} flag, simply omit a value for the variable,
  e.g.~\texttt{-\/-variable\ draft}).
\end{itemize}
...
\tightlist
\item[input formats]
\texttt{latex}, \texttt{org}, \texttt{textile}, \texttt{html}
(environments, \texttt{\textbackslash{}ref}, and
\texttt{\textbackslash{}eqref} only)
\item[output formats]
...
For \texttt{epub3} output, you can mark up the header that corresponds
